...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    AbstractBootstrap(AbstractBootstrap<B, C> bootstrap) {
        ...
        //因为传入的 bootstrap 参数的 options 和 attrs 属性，可能在另外的线程被修改( 例如#option(hannelOption<T> option, T value) 方法)
     ...
     */
    // attrs 可以理解成 java.nio.channels.SelectionKey 的 attachment 属性，并且类型为 Map 。
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        ...
        // 这里的逻辑非常清晰：注册已经完成那么我就绑定，没有完成就注册监听器然后监听注册成功后绑定
        if (regFuture.isDone()) { // 完成
            ...
            //这里的listener会在 AbstractChannel.AbstractUnsafe.register0() 调用了 safeSetSuccess() 方法然后被调用
            //因为注册完毕了
    ...
    final ChannelFuture initAndRegister() {
        ...
        //这里就是出现异常了，判断逻辑之所以不一样的区别在于是否注册成功
        //注册成功了就正常关闭调用close()
        //否则强制关闭closeForcibly()
